...
LIGHTNING_CHARS = [
    ...
    "+",
    "#",
]  # Different intensity characters [dimmest -> brightest]
...
class LightningBolt:
    ...
    def update(self):
        ...
        if self.is_growing and (
            current_time - self.last_growth_time >= LIGHTNING_GROWTH_DELAY
        ):
            ...
            if len(self.segments) < self.target_length and last_y < self.max_y - 1:
                ...
                for i in range(branches):
                    ...
                    if i == 0:
                        next_primary_x = next_x  # Store first path pos
                    current_x = next_x
                    added_segment = True

                # --- Add Secondary Forks --- #
                if random.random() < FORK_CHANCE:
                    fork_offset = random.randint(
                        -FORK_HORIZONTAL_SPREAD, FORK_HORIZONTAL_SPREAD
                    )
                    if fork_offset == 0:
                        fork_offset = random.choice([-1, 1])
            ...
            if (
                not added_segment
                or len(self.segments) >= self.target_length
                or last_y >= self.max_y - 1
            ):
            ...
            if new_segments_this_step:
                ...
                unique_new = list(
                    {(s[0], s[1]): s for s in new_segments_this_step}.values()
                )
...
def setup_colors(rain_color_str="cyan", lightning_color_str="yellow"):
    ...
    if curses.has_colors():
        ...
        lightning_fg = CURSES_COLOR_MAP.get(
            lightning_color_str.lower(), curses.COLOR_YELLOW
        )
        # ------------------------------------------------ #
...
def simulate_rain(stdscr, rain_color_str="cyan", lightning_color_str="yellow"):
    ...
    while True:
        ...
        if (
            is_thunderstorm
            and len(active_bolts) < 3
            and random.random() < LIGHTNING_CHANCE
        ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Simulates rain and thunderstorms in the terminal."
    )
    ...
    try:
        ...
        print(
            "Terminal might not fully support curses features (like color/attributes)."
        )
